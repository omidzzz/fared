...
const SHOP_ITEMS = [
  { label: 'لباس‌ها',    href: '/shop/clothes',     color: 'var(--chakra-root)'   },
  { label: 'شمع‌ها',    href: '/shop/candles',     color: 'var(--chakra-sacral)' },
  { label: 'لوازم جانبی',href: '/shop/accessories', color: 'var(--chakra-solar)'  },
  { label: 'تورها',      href: '/shop/tours',       color: 'var(--chakra-heart)'  },
  { label: 'سنگ‌ها',     href: '/shop/stones',      color: 'var(--chakra-throat)' },
  { label: 'دوره‌ها',    href: '/shop/courses',     color: 'var(--chakra-third)'  },
  { label: 'منتورشیپ', href: '/shop/mentorship',  color: 'var(--chakra-crown)'  },
]

const DESKTOP_NAV_LINKS = [
  { label: 'بلاگ',   href: '/blog',      fa: true },
  { label: 'تحریریه', href: '/tahrirye', fa: true },
  { label: 'انجمن',   href: '/forum',     fa: true },
  { label: 'تماس با ما', href: '/contact', fa: true },
]
...
const MOBILE_MORE_LINKS = [
  { label: 'راهنمای انرژی', href: '/energy-guide' },
  { label: 'بلاگ',      href: '/journal'      },
  ...
  return (
    <>
      <header
        ...
        style={{
          height: 72,
          filter: scrolled ? 'drop-shadow(0 4px 20px rgba(0,0,0,0.50))' : 'none',
          transition: 'filter 0.3s ease',
        }}
      >
        {/* Frosted glass background — 72px */}
        <div
        ...
        {/* Decorative wavy gold edge — replaces the old 1px gradient line.
            Source asset: public/images/header-new.svg (native viewBox
            0 0 2543.72 1533.57, artwork only in the top band y≈-99..141).
            Rendered with viewBox 0 0 2543.72 150 + preserveAspectRatio="none"
            so it stretches full-width across the header bottom, recolored from
            the raw peach to the header gold and softened with opacity so it
            fades into the page. Sits just below/overlapping the bar's bottom
            edge, below the nav content, pointer-events none. */}
        <svg
        ...
        {/* Nav content — 72px, vertically centered.
            justify-between pushes the logo group to the RTL start (right) and
            the icons group to the RTL end (left); the desktop <nav> is
            absolutely centered so it stays in the true middle of the bar
            regardless of the side groups' widths. */}
        <div className="relative z-10 h-[72px] flex items-center justify-between px-5 lg:px-10">

          {/* RIGHT (RTL start): Logo + brand */}
          <div className="flex items-center flex-shrink-0">
            <Link href="/" className="flex items-center gap-2">
              <div
                className="w-8 h-8 rounded-full flex items-center justify-center"
                style={{ background: 'rgba(254,203,125,0.15)', border: '1px solid rgba(254,203,125,0.40)' }}
              >
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path
                    d="M8 2 C8 2 4 5 4 9 C4 11.5 5.5 13 8 14.5 C10.5 13 12 11.5 12 9 C12 5 8 2 8 2Z"
                    stroke="#FECB7D" strokeWidth="1" fill="rgba(254,203,125,0.15)"
                  />
                  <circle cx="8" cy="9" r="2" stroke="#FECB7D" strokeWidth="0.8" fill="none"/>
                </svg>
              </div>
              <div className="hidden sm:block">
                <p className="font-farsi text-sm leading-none" style={{ color: 'var(--gold-accent)' }}>فارد</p>
                <p className="font-farsi text-[9px] text-[--text-muted]">تجارت معنوی</p>
              </div>
            </Link>
          </div>
          ...
          <nav className="hidden lg:flex items-center gap-6 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
              <Link href="/">
                <span className={`font-farsi text-sm transition-colors ${pathname === '/' ? 'text-[--gold-accent]' : 'text-[--text-secondary] hover:text-[--text-primary]'}`}>
                  خانه
                </span>
              </Link>
              ...
              >
                <button className="flex items-center gap-1 font-farsi text-sm text-[--text-secondary] hover:text-[--text-primary] transition-colors py-2">
                  فروشگاه
                  <svg
                      ...
                      {SHOP_ITEMS.map((item) => (
                        <Link key={item.href} href={item.href} onClick={() => setShopOpen(false)}>
                          <div className="flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors hover:bg-white/[0.07]">
                            ...
                            />
                            <span className="font-farsi text-sm text-[--text-secondary] hover:text-[--text-primary] transition-colors">
                              {item.label}
                            </span>
                          </div>
                        </Link>
              ...
              {DESKTOP_NAV_LINKS.map((link) => (
                <Link key={link.href} href={link.href}>
                  <span className={`text-sm transition-colors ${link.fa ? 'font-farsi' : 'font-body'} ${pathname === link.href ? 'text-[--gold-accent]' : 'text-[--text-secondary] hover:text-[--text-primary]'}`}>
                    {link.label}
                  </span>
                </Link>
              ))}
          ...
          {/* LEFT (RTL end): Search + Icons — desktop only */}
          <div className="hidden lg:flex items-center gap-2 flex-shrink-0">
            ...
            <div className="relative w-48 xl:w-64">
              ...
              <div className="absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.35)" strokeWidth="1.8">
            ...
            >
              {isLoggedIn && user?.avatar ? (
                <div className="w-7 h-7 rounded-full overflow-hidden relative">
                  <Image src={user.avatar} alt={user.nameFA} fill className="object-cover" />
                </div>
              ) : (
            ...
            {/* Cart */}
            <Link href="/cart" className="relative p-2 rounded-lg transition-colors hover:bg-white/[0.06]">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" strokeWidth="1.5">
              ...
              {totalItems > 0 && (
                <span
                  ...
                  style={{ width: 17, height: 17, background: 'var(--gold-accent)', color: 'var(--cosmic-dark)', fontSize: 10, fontWeight: 700 }}
                >
