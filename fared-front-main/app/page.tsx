...
import type { Stone, Candle, Accessory, ClothingItem } from '@/lib/mock-data'
...
const Divider = () => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '8px 60px' }}>
  ...
  return (
    <>
      ...
      <div className="hidden lg:block">
        ...
        {/* HERO SECTION — with its own background image */}
        <section style={{
          backgroundImage: 'url(/images/hero-backgrounds/home-page-hero.webp)',
          backgroundSize: 'cover', backgroundPosition: 'center 42%',
          minHeight: '100vh', position: 'relative', display: 'flex', flexDirection: 'column', overflow: 'visible',
        }}>
          ...
          <div className="relative z-10 flex items-center flex-1">
            <section className="flex items-center w-full" dir="ltr">
              <div className="flex flex-col justify-center pl-16 xl:pl-24 w-1/2 pr-8" dir="rtl">
                <h1 className="font-farsi font-bold leading-snug mb-4" style={{
                  fontSize: 'clamp(2.8rem, 5vw, 5.5rem)',
                  background: 'linear-gradient(140deg, #FECB7D 0%, #f5dfa0 40%, #f0ebe3 65%, #FECB7D 100%)',
                  WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text',
                }}>انرژی کیهان<br/>در دستان توست</h1>
                <div className="flex items-center gap-2 mb-4 opacity-70" style={{ direction: 'ltr' }}>
                ...
                <p className="font-farsi text-[--text-secondary] leading-relaxed mb-8" style={{ fontSize: 'clamp(0.85rem, 1.5vw, 1rem)', maxWidth: 420 }}>
          ...
          <div className="relative z-10" style={{ marginTop: 80, marginBottom: -60, padding: '0 40px' }}>
            <FeatureBadges />
          </div>
        </section>
            ...
            <div key={section.viewAllHref}>
              ...
              <section className="py-8">
                <div className="text-center mb-8 px-4" dir="rtl">
                  <div className="flex items-center justify-center gap-[18px] mb-3">
                    <span style={{ color: 'var(--gold-accent)', fontSize: 18, letterSpacing: 4, filter: 'drop-shadow(0 0 7px rgba(231,193,111,0.7))' }}>❖</span>
                    <h2 style={{ fontFamily: 'var(--fa)', fontSize: 33, fontWeight: 700, color: 'var(--gold-accent)', textShadow: '0 0 16px rgba(231,193,111,0.3)', whiteSpace: 'nowrap' }}>{section.title}</h2>
                    <span style={{ color: 'var(--gold-accent)', fontSize: 18, letterSpacing: 4, filter: 'drop-shadow(0 0 7px rgba(231,193,111,0.7))' }}>❖</span>
                  </div>
                  <p className="font-farsi" style={{ fontWeight: 300, color: 'rgba(255,248,238,0.82)', marginTop: 13, fontSize: 17 }}>{section.subtitle}</p>
                </div>
      ...
      {/* ── MOBILE LAYOUT ── */}
      <section className="block lg:hidden px-4 pt-[88px] pb-4">
        <div className="relative rounded-3xl overflow-hidden aspect-[4/3]"
             style={{ boxShadow: '0 0 0 1px rgba(254,203,125,0.25), 0 0 24px rgba(254,203,125,0.12), 0 0 48px rgba(254,203,125,0.06)', border: '1px solid rgba(254,203,125,0.30)' }}>
          <Image src="/images/hero-backgrounds/home-page-hero.webp" alt="" fill className="object-cover object-top" priority
                 onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }}/>
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent"/>
          <HeroShimmer/>
          <div className="absolute bottom-0 left-0 right-0 z-20 p-6">
            <h1 className="font-display text-3xl text-[var(--text-primary)]" dir="rtl">انرژی کیهان<br/>در دستان توست</h1>
            <Link href="/shop">
              <button className="mt-4 px-4 py-2 rounded-lg min-h-9 font-body text-xs font-semibold transition-all duration-300"
                      style={{ background: 'var(--gold-accent)', color: 'var(--cosmic-dark)' }}>کشف مجموعه</button>
            </Link>
          </div>
        </div>
      </section>
