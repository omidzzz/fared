...
## Bug Fix — Horizontal Scrollbar on Mobile Homepage
### Issue
When scrolling the homepage on mobile devices, a horizontal scrollbar would appear and the screen would jump.

### Root Cause
The hero section had `overflow: 'visible'` which allowed content to overflow horizontally. The FeatureBadges component was causing horizontal overflow on small screens.

### Fix Applied
- **Hero section**: Changed `overflow: 'visible'` to `overflow: 'hidden'` in `app/page.tsx`
- **FeatureBadges container**: Added `maxWidth: '100%'` and `overflowX: 'hidden'` to prevent horizontal overflow
- **Result**: No more horizontal scrollbar, smooth vertical scrolling on mobile
